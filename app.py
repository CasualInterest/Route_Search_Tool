...
def parse_any_date(series: pd.Series) -> pd.Series:
    s = series.copy()
    s = s.astype(str).str.strip().replace({'': np.nan, 'nan': np.nan, 'NaN': np.nan, 'None': np.nan})
...
with st.expander('🛠 Date/Origin drop diagnostics (click to expand)'):
    if Path(MASTER_CSV).exists():
        _raw = pd.read_csv(MASTER_CSV, dtype=str)
        _eff_bad = _raw['Eff Date'].notna().sum() - parse_any_date(_raw['Eff Date']).notna().sum() if 'Eff Date' in _raw.columns else 0
        _term_bad = _raw['Term Date'].notna().sum() - parse_any_date(_raw['Term Date']).notna().sum() if 'Term Date' in _raw.columns else 0
        _origin_blank = (_raw['Origin'].astype(str).str.strip() == '').sum() if 'Origin' in _raw.columns else 0
        st.write(f'Unparsed Eff Date rows: {_eff_bad}, Unparsed Term Date rows: {_term_bad}, Blank Origin (if any present in master): {_origin_blank}')
        if _eff_bad or _term_bad or _origin_blank:
            bad_rows = _raw.loc[
                (parse_any_date(_raw.get('Eff Date', pd.Series(dtype=str))).isna()) |
                (parse_any_date(_raw.get('Term Date', pd.Series(dtype=str))).isna()) |
                (_raw.get('Origin', pd.Series(dtype=str)).astype(str).str.strip() == ''),
                DISPLAY_COLS
            ]
